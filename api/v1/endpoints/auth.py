...
async def auth_update_settings(request: Request, body: AuthSettingsRequest):
    ...
    if target_enabled:
        if password or confirm:
            if stored_password_exists:
                return JSONResponse(
                    ...
                    content={
                        ...
                        "message": "A senha do administrador já está definida. Ative a autenticação e use a função de alterar senha para atualizá-la",
                    },
                )
            if not password:
                return JSONResponse(
                    status_code=400,
                    content={"error": "password_required", "message": "Por favor, insira a senha do administrador que deseja definir"},
                )
            if password != confirm:
                return JSONResponse(
                    status_code=400,
                    content={"error": "password_mismatch", "message": "As senhas inseridas não coincidem"},
                )
            ...
            return JSONResponse(
                status_code=400,
                content={"error": "password_required", "message": "Por favor, defina uma senha antes de ativar o login por senha"},
            )
        else:
            ...
            if not is_valid_session:
                if not current_password:
                    return JSONResponse(
                        status_code=400,
                        content={"error": "current_required", "message": "Por favor, insira a senha atual antes de reativar a autenticação"},
                    )
                ...
                if not verify_stored_password(current_password):
                    ...
                    return JSONResponse(
                        status_code=401,
                        content={"error": "invalid_password", "message": "Senha atual incorreta"},
                    )
                clear_rate_limit(ip)
    else:
        if current_enabled:
            ...
            if not is_valid_session:
                if not current_password:
                    return JSONResponse(
                        status_code=400,
                        content={"error": "current_required", "message": "Por favor, insira a senha atual antes de desativar a autenticação"},
                    )
                ip = get_client_ip(request)
                if not check_rate_limit(ip):
                    return JSONResponse(
...
async def auth_login(request: Request, body: LoginRequest):
    ...
    if not password:
        return JSONResponse(
            status_code=400,
            content={"error": "password_required", "message": "Por favor, insira a senha"},
        )
    ...
    if not password_set:
        ...
        if not verify_password(password):
            ...
            return JSONResponse(
                status_code=401,
                content={"error": "invalid_password", "message": "Senha incorreta"},
            )
...
async def auth_change_password(body: ChangePasswordRequest):
    ...
    if not current:
        return JSONResponse(
            status_code=400,
            content={"error": "current_required", "message": "Por favor, insira a senha atual"},
        )
    if new_pwd != new_confirm:
        return JSONResponse(
            status_code=400,
            content={"error": "password_mismatch", "message": "As novas senhas inseridas não coincidem"},
        )
