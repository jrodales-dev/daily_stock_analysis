  ...
  return (
    <AppPage className="space-y-5">
      <PageHeader
        ...
        title="Alert Center"
        description="Manage event alerts, daily technical indicators, watchlists, portfolio/account integration, and market sentiment rules, perform one-time tests, and view the trigger history of backend evaluation tasks."
      />
      ...
      <Card title="Notification Attempt Log" subtitle="Notification Result" variant="bordered" padding="md">
        {notificationsLoading ? <Loading label="Loading notification attempt log" /> : null}
        {!notificationsLoading && notifications.length === 0 ? (
          <EmptyState
            icon={<BellRing className="h-6 w-6" />}
            title="No notification attempt records"
            description="No notification attempt details to display; alert triggers will still be sent according to the configured notification channels."
          />
        ) : null}
        {!notificationsLoading && notifications.length > 0 ? (
