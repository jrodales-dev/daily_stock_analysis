...
const LoginPage: React.FC = () => {
  ...
  useEffect(() => {
    document.title = 'Login - DSA Terminal';
  }, []);
  ...
  const onSubmit = async (data: LoginForm) => {
    setApiError(null);
    try {
      // In a real OAuth2 implementation, we'd send FormData with username and password.
      // Here we assume login hook will handle it or we adapt. For now we use the hook as is.
      await login(data.password, ''); // Adapting to existing hook signature (password, token)
      toast.success('Login bem-sucedido!');
      navigate(redirect, { replace: true });
    } catch (err: any) {
      setApiError(err.message || 'Credenciais inválidas');
        ...
        >
          ...
          <div className="pointer-events-auto relative flex flex-col overflow-hidden rounded-3xl border border-[var(--login-border-card)] bg-[var(--login-bg-card)]/80 p-8 shadow-2xl backdrop-blur-xl">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
              ...
              {apiError && (
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }}>
                  <SettingsAlert title="Falha de Autenticação" message={apiError} variant="error" />
                </motion.div>
              )}
            ...
            <div className="mt-4 text-center text-sm text-[var(--login-text-muted)]">
              Não tem uma conta? <Link to="/register" className="text-[var(--login-brand-end)] hover:underline">Registre-se</Link>
            </div>
          </div>
        </motion.div>
      </div>
