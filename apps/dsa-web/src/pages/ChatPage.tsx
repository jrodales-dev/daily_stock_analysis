...
const QUICK_QUESTIONS = [
  { label: 'Analisar Moutai com a Teoria de Chan', skill: 'chan_theory' },
  { label: 'CATL usando Teoria das Ondas', skill: 'wave_theory' },
  { label: 'Analisar tendência da BYD', skill: 'bull_trend' },
  { label: 'Oscilação em caixa SMIC', skill: 'box_oscillation' },
  { label: 'Analisar Tencent hk00700', skill: 'bull_trend' },
  { label: 'Ciclo de emoção East Money', skill: 'emotion_cycle' },
];
...
const ChatPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    void systemConfigApi.getConfig(false)
      ...
      .catch((error) => {
        ...
        setContextCompressionError(parsed.message || 'Unable to read context compression configuration');
  ...
  const updateContextCompressionEnabled = useCallback(
    async (nextEnabled: boolean) => {
      ...
      try {
        ...
        setContextCompressionError(parsed.message || 'Failed to save context compression settings');
      } finally {
  ...
  // Gerencia parâmetros Query da Barra URL: ?stock=1&name=Amazon...
  useEffect(() => {
  ...
  const handleSend = useCallback(
    async (overrideMessage?: string, overrideSkillIds?: string[]) => {
      ...
      const usedSkillNames = usedSkillIds.length > 0 ? getSkillNames(usedSkillIds) : ['Geral'];
  ...
  const downloadMessageAsMarkdown = useCallback((msg: Message) => {
    ...
    const heading = msg.role === 'user' ? '# User Message' : `# AI Reply${skillLabel ? ` · ${skillLabel}` : ''}`;
  ...
  const getCurrentStage = (steps: ProgressStep[]): string => {
    if (steps.length === 0) return 'Conectando...';
    const last = steps[steps.length - 1];
    if (last.type === 'thinking') return last.message || 'A IA está pensando...';
    if (last.type === 'tool_start')
      return `${last.display_name || last.tool}...`;
    if (last.type === 'tool_done')
      return `${last.display_name || last.tool} concluído`;
    if (last.type === 'generating')
      return last.message || 'Gerando análise final...';
    return 'Processando...';
  };
  ...
  const renderThinkingBlock = (msg: Message) => {
    ...
    const summary = `${toolSteps.length} chamadas de ferramenta · ${totalDuration.toFixed(1)}s`;
    ...
    return (
      ...
      >
        ...
        <span className="flex items-center gap-1.5">
          <span className="opacity-60">Processo de Pensamento</span>
    ...
    <div className="mb-3 pl-5 border-l border-border/40 space-y-1.5 animate-fade-in">
      {steps.map((step, idx) => {
        ...
        if (step.type === 'thinking') {
          text = step.message || `Passo ${step.step}: Pensando`;
          ...
          text = step.message || 'Gerando análise';
      ...
      <ScrollArea testId="chat-session-list-scroll" viewportClassName="p-3">
        {sessionsLoading ? (
          ...
          <DashboardStateBlock
            compact
            title="Sem histórico"
            description="Suas sessões de chat aparecerão aqui quando você iniciar."
            className="rounded-2xl border border-dashed border-border/50 bg-surface/30"
          />
        ) : (
  ...
  return (
    ...
    >
      ...
      <ConfirmDialog
        ...
        message="Esta sessão será permanentemente excluída. Tem certeza?"
        confirmText="Excluir"
        cancelText="Cancelar"
        isDanger
      ...
      <div className="flex h-full min-w-0 flex-1 flex-col overflow-hidden">
        <header className="mb-4 flex-shrink-0 space-y-3">
          <div className="flex items-start justify-between gap-4">
            ...
            {messages.length > 0 && (
              <div className="flex flex-shrink-0 flex-wrap items-center justify-end gap-2">
                <Tooltip content="Exportar sessão para arquivo Markdown">
                  <span className="inline-flex">
                    <Button
                      variant="action-primary"
                      size="sm"
                      onClick={() => downloadSession(messages)}
                      aria-label="Exportar sessão para arquivo Markdown"
                    >
                      <svg
                        className="w-4 h-4"
                        fill="none"
                ...
                <Tooltip content="Enviar para bot de notificação/e-mail configurado">
                  <span className="inline-flex">
                    <Button
                      variant="action-primary"
                      size="sm"
                      ...
                      onClick={async () => {
                        ...
                        try {
                          ...
                          showSendFeedback({ type: 'success', message: 'Enviado para o canal de notificação' }, 3000);
                        } catch (err) {
                          const parsed = getParsedApiError(err);
                          showSendFeedback({
                            type: 'error',
                            message: parsed.message || 'Falha ao enviar',
                          }, 5000);
                        } finally {
                          setSending(false);
                        }
                      }}
                      aria-label="Enviar para bot de notificação/e-mail configurado"
                    >
                      {sending ? (
                        <svg
          ...
          <p className="text-secondary-text text-sm">
            Peça à IA para analisar ações individuais e obter relatórios de decisão em tempo real com base em diferentes estratégias.
          </p>
          {sendToast ? (
            <InlineAlert
              variant={sendToast.type === 'success' ? 'success' : 'danger'}
              title={sendToast.type === 'success' ? 'Enviado com sucesso' : 'Falha ao enviar'}
          ...
          >
            {messages.length === 0 && !loading ? (
              <div className="flex h-full items-center justify-center">
                <EmptyState
                  title="Iniciar Chat"
                  description="Digite 'Analisar 600519' ou 'Vale a pena comprar Moutai agora?'. A IA usará ferramentas de dados em tempo real para gerar um relatório de decisão."
                      ...
                      <div className="relative">
                        <div className="chat-message-actions">
                          ...
                          <button
                            ...
                            className="chat-copy-btn"
                            aria-label="Export this message to Markdown"
                          >
                            Exportar
                          </button>
                        </div>
          ...
          <div className="border-t border-white/6 bg-card/88 p-4 md:p-6 relative z-20">
            <div className="space-y-3">
              ...
              {isFollowUpContextLoading ? (
                <InlineAlert
                  variant="info"
                  title="Carregando contexto adicional"
                  message="Carregando contexto de análise histórica; você já pode enviar perguntas de acompanhamento diretamente."
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-white/6 bg-surface/25 px-3 py-2">
                ...
                >
                  ...
                  />
                  <span className="font-medium">Compressão de Contexto</span>
                  <span className="text-xs text-muted-text">Economizar tokens em sessões longas</span>
                </label>
              ...
              {contextCompressionError ? (
                <InlineAlert
                  variant="danger"
                  title="Configurações de compressão de contexto não salvas"
            ...
            {skills.length > 0 && (
              ...
              <div className="flex items-end gap-3">
                <textarea
                  ...
                  placeholder="ex: Analisar 600519 / Vale a pena comprar Moutai agora? (Enter para enviar, Shift+Enter para nova linha)"
                  disabled={loading}
                  rows={1}
