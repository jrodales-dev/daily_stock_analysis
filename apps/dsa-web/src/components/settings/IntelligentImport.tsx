...
function getConfidenceMeta(confidence: 'high' | 'medium' | 'low') {
  if (confidence === 'high') {
    return { label: 'Alta', badge: 'success' as const };
  }
  if (confidence === 'low') {
    return { label: 'Baixa', badge: 'warning' as const };
  }
  return { label: 'Média', badge: 'default' as const };
}
...
export const IntelligentImport: React.FC<IntelligentImportProps> = ({
  ...
  const handleImageFile = useCallback(
    async (file: File) => {
      ...
      if (!IMG_EXT.includes(ext)) {
        setError('Somente as imagens nos formatos JPG, PNG, WebP e GIF são aceitos');
        return;
      }
      if (file.size > IMG_MAX) {
        setError('Tamanho do Arquivo não pode exceder 5MB');
        return;
      }
      setError(null);
      setIsLoading(true);
      try {
        ...
        let fallback = 'Reconhecimento falhou. Verifique e tente novamente';
        if (err?.response?.status === 429) fallback = 'Muitas requisições sequenciais. Acalme as coisas...';
        else if (err?.code === 'ECONNABORTED') fallback = 'Timeout da Requisição. Faltou internet?';
  ...
  const handleDataFile = useCallback(
    async (file: File) => {
      if (file.size > FILE_MAX) {
        setError('Arquivos menores de 2MB apenas');
        return;
      }
      setError(null);
      setIsLoading(true);
      try {
        ...
        const parsed = getParsedApiError(e);
        setError(parsed.message || 'Falha no parser do arquivo');
      } finally {
        setIsLoading(false);
      }
    },
  ...
  const handlePasteParse = useCallback(() => {
    ...
    if (new Blob([t]).size > TEXT_MAX) {
      setError('A transferência via Paste não aguenta mais que 100kb.');
      return;
    }
    setError(null);
    setIsLoading(true);
  ...
  const mergeToWatchlist = useCallback(async () => {
    ...
    if (!configVersion) {
      setError('Aguarde as Definições do Sistema iniciarem');
      return;
    }
    ...
    try {
      ...
      if (e instanceof SystemConfigConflictError) {
        await onMerged(value);
        setError('Definições salvas. Selecione Mesclar Novamente');
      } else {
        setError(e instanceof Error ? e.message : 'Mescla obteve Falha');
      }
    } finally {
      setIsMerging(false);
    }
  ...
  return (
    <div className="space-y-4">
      <div className="settings-surface-panel settings-border-strong rounded-xl border p-4 shadow-soft-card">
        <p className="text-sm font-medium text-foreground">Suporte para Imagens, Arquivos CSV/Excel e Área de Transferência</p>
        <p className="mt-1 text-xs leading-5 text-secondary-text">
          A IA usa Vision para ler as imagens. Aconselhamos testar a Imagem antes.
        </p>
      </div>
      ...
      >
        ...
        <div className="flex flex-col gap-2 sm:flex-row">
          <textarea
            placeholder="...Ou cole o Texto Exato do Clipboard..."
      ...
      {isLoading && <p className="text-sm text-secondary-text">Processando...</p>}
      {error && (
        <InlineAlert
          variant="danger"
          message={error}
      ...
      {items.length > 0 && (
        <div className="space-y-2">
          <InlineAlert
            variant="warning"
            message="Certifique-se de comparar e conferir as informações com as importadas."
            className="rounded-xl px-3 py-2 text-xs shadow-none"
          />
          <div className="flex items-center justify-between">
            <span className="text-xs text-secondary-text">
              Total {validCount} com mescla. Sendo {checkedCount} aprovadas
            </span>
          ...
          >
            {isMerging ? 'Gravando...' : 'Fazer o Merge em Stock Pool'}
          </Button>
        </div>
      )}
    </div>
